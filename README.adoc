...
To install {ProjectShort} we need to install an OperatorSource that container the Service Telemetry Operator and the Smart Gateway Operator.
...
[NOTE]
The AMQ Certificate Manager is installed globally for all namespaces, so the `namespace` value provided is `openshift-operators`. You may not see your `amq7-cert-manager.v1.0.0` ClusterServiceVersion in the `service-telemetry` namespace for a few minutes until the processing gets to our namespace.
